...
function ai1HitCallback(ai1Hit, bulletHit)
{
    // Reduce health of player
    if (bulletHit.active === true && ai1Hit.active === true)
    {
        ai1Hit.health = ai1Hit.health - 1;
        console.log("ai1 hp: ", ai1Hit.health);

        // Kill hp sprites and kill ai1 if health <= 0
        if (ai1Hit.health == 0)
        {
            ai1.destroy();
            // Game over state should execute here
...
//Win Game
/*function win (player, strawberry, gameObject)
{
    if (Phaser.Geom.Intersects.RectangleToRectangle(this.player.getBounds(), this.strawberry.getBounds()))
    {
